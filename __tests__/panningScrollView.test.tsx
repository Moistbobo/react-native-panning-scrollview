...
describe('PanningScrollViewTests', function() {
  it('renders', async() => {
    expect(render(<PanningScrollView useNativeDriver={false}>
      {dummyContent}
    </PanningScrollView>)).toBeTruthy();
  })

  it('renders a header image if provided', async ()=>{
    const {getByTestId} = render(<PanningScrollView headerImage={headerImg} useNativeDriver={false}>{dummyContent}</PanningScrollView>);

    expect(getByTestId('image-header')).toBeTruthy();
  })

  it('renders placeholder if header is not provided', async ()=>{
    const {getByTestId, queryByTestId} = render(<PanningScrollView useNativeDriver={false}>{dummyContent}</PanningScrollView>);
  ...
  it('renders a badge if provided', async ()=>{
    const {getByTestId} = render(<PanningScrollView badgeImage={badgeImg} useNativeDriver={false}>{dummyContent}</PanningScrollView>);

    expect(getByTestId('image-badge')).toBeTruthy();
  })

  it('does not render a badge if not provided', async()=>{
    const {queryByTestId} = render(<PanningScrollView useNativeDriver={false}>{dummyContent}</PanningScrollView>);
